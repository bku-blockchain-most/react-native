...
class VillageList extends Component {
  ...
  _renderHeader(section, index, isActive, sections) {
    return (
      <Animatable.View duration={300} transition="backgroundColor" style={{backgroundColor: isActive ? 'rgba(129, 207, 224, 1)' : 'rgba(255,255,255,1)', flexDirection: 'row'}}>
        <Text style={{fontSize: 18, flex: 8}}>{section.village_name}</Text>

        <Image style={styles.imageStyle} source={require('./images/down_arrow2.png')} />
      </Animatable.View>
    );
  }
      ...
      <View style={{flex: 1}}>
        <ScrollView>
          ...
          <View style={{alignItems: 'center'}}>
            <Button2
              buttonPress={() => {
                this.onButtonPress(this.props.text, this.state.checked);
              }}>
